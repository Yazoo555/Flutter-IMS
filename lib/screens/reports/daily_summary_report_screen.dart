...
class _DailySummaryReportScreenState extends State<DailySummaryReportScreen> {
  ...
  Future<void> _fetchReport() async {
    ...
    try {
      final data = await supabase.rpc('get_daily_summary_with_value', params: { 'p_item_id': _selectedItemId, 'p_days_back': _daysBack });
      if (!mounted) return;
      final summaries = (data as List).map((e) => DailyStockSummary.fromJson(e as Map<String, dynamic>)).toList();
      setState(() { _summaries = summaries; _isLoading = false; });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Daily Summary', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 18, color: c.textPrimary)),
        backgroundColor: c.surface, elevation: 0,
        iconTheme: IconThemeData(color: c.textPrimary),
        actions: [
          IconButton(icon: const Icon(Icons.download_rounded),
            onPressed: (_summaries.isEmpty || _selectedItemId == null) ? null : () {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Days History:', style: TextStyle(fontSize: 14, color: c.textSecondary)),
          DropdownButton<int>(
            value: _daysBack, underline: const SizedBox(),
            dropdownColor: c.surface,
            ...
            onChanged: (val) { if (val != null) { setState(() => _daysBack = val); _fetchReport(); } },
          ),
        ]),
      ]),
    );
  ...
  Widget _buildBody() {
    ...
    if (_summaries.isEmpty) return Center(child: Text('No daily summary data found.', style: TextStyle(color: c.textSecondary)));

    return ListView.builder(
      padding: const EdgeInsets.all(16), itemCount: _summaries.length,
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Container(width: 52, height: 56, decoration: BoxDecoration(color: c.primaryLight, borderRadius: BorderRadius.circular(10)),
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Text(DateFormat('dd').format(s.date), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: AppTheme.primary)),
                  Text(DateFormat('MMM').format(s.date), style: const TextStyle(fontSize: 11, color: AppTheme.primary, fontWeight: FontWeight.w600)),
                ])),
              const SizedBox(width: 12),
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(DateFormat('EEEE').format(s.date), style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: c.textPrimary)),
                const SizedBox(height: 2),
                Wrap(spacing: 6, children: [
                  if (s.totalPurchasedQty > 0) _qtyBadge('IN', s.totalPurchasedQty, const Color(0xFF10B981)),
                  if (s.totalSoldQty > 0) _qtyBadge('OUT', s.totalSoldQty, AppTheme.errorColor),
                ]),
              ]),
              const Spacer(),
            ...
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: _valueStat('Purchase Value', 'Rs ${_fmt.format(s.purchaseValue)}', const Color(0xFF10B981))),
              const SizedBox(width: 16),
              Expanded(child: _valueStat('Sales Value', 'Rs ${_fmt.format(s.salesValue)}', const Color(0xFF6366F1))),
            ]),
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: _valueStat('Closing Stock', '${s.closingStockQty % 1 == 0 ? s.closingStockQty.toInt() : s.closingStockQty} units', const Color(0xFFF59E0B))),
              const SizedBox(width: 16),
              Expanded(child: _valueStat('Stock Value', 'Rs ${_fmt.format(s.closingStockValue)}', const Color(0xFF3B82F6), bold: true)),
            ]),
          ])),
        );
      },
  ...
  Widget _qtyBadge(String label, double val, Color color) {
    return Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
      ...
      child: Text('$label ${val % 1 == 0 ? val.toInt() : val}', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: color)));
  }
  ...
  Widget _profitPill(double profit) {
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
      ...
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Rs ${_fmt.format(profit)}', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: color)),
      ]));
  }

  Widget _valueStat(String label, String value, Color valueColor, {bool bold = false}) {
    final c = context.colors;
    return Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      decoration: BoxDecoration(color: valueColor.withOpacity(0.08), borderRadius: BorderRadius.circular(10), border: Border.all(color: valueColor.withOpacity(0.2))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(label, style: TextStyle(fontSize: 11, color: c.textSecondary, letterSpacing: 0.2)),
        const SizedBox(height: 6),
        Text(value, style: TextStyle(fontSize: 14, fontWeight: bold ? FontWeight.w800 : FontWeight.w700, color: valueColor), overflow: TextOverflow.ellipsis),
      ]));
  }
}
